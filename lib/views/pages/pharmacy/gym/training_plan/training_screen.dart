...
import '../../../../../requests/allcategory.service.dart';
import '../../../../../utils/color.dart';
import '../../../../../utils/utils.dart';
import '../../../home.page.dart';
import '../custom/drawer/drawer_menu.dart';
import '../discover/DiscoverScreen.dart';
import '../exerciseDays/exercise_days_status_screen.dart';
import '../exerciselist/ExerciseListScreen.dart';
import '../interfaces/topbar_clicklistener.dart';
import '../quarantineathome/QuarantineAtHomeScreen.dart';
import '../setWeeklyGoal/set_weekly_goal_screen.dart';
import '../workoutHistory/workout_history_screen.dart';
...
class _TrainingScreenState extends State<TrainingScreen>
              ...
              child: Container(
                ...
                child: ClipRRect(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      image: DecorationImage(
                          image: (allPlanDataList[index].catImage.contains("http") || allPlanDataList[index].catImage.contains("https"))?
                              NetworkImage(allPlanDataList[index].catImage):
                          AssetImage((allPlanDataList[index].catImage !=
                                  Constant.title)
  ...
  _getAllPlanList() async {
    var data = await DataBaseHelper().getHomePlanData();
    if(kDebugMode){
      log(jsonEncode(data));
    }
    // allPlanDataList = data;
    allCategory = await AllCategoryRequest().getAllCategory(vendorTypeId: 4);
    allPlanDataList = [];
    if(allCategory.data.isEmpty){
      return;
    }
    for(int index=0; index<= allCategory.data.length-1; index++){
      allPlanDataList.add(data.firstWhere((element) => element.catId == allCategory.data[index].idgym));
      if(allCategory.data[index].subcategories.isNotEmpty){
        for(int index2=0; index2<=allCategory.data[index].subcategories.length-1; index2++){
          // print(jsonEncode(allCategory.data[index].subcategories));
          var sub = data.firstWhere((element) => element.catId == allCategory.data[index].subcategories[index2].idgym);
          sub.catImage = allCategory.data[index].subcategories[index2].urlImage;
          allPlanDataList.add(sub);
        }
      }
    }
